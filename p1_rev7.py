...
class psi: 
    ...
    def create_tri(lx1,lx2,ly,dx,dy, w, tolerance):
        '''This function is used to construct a tri-diagonal matrix
        '''
        beta = dx/dy 
        lx = 2*lx1+lx2 + 2*dx #length in x-direction (Width)
        nx = int(lx/dy) #number of grid points x-dir
        ny = int(ly/dy) #number of grid points y-dir
        PSI = sp.zeros((ny,nx))
...
class grid:
    ...
    def create_mesh(lx1, lx2, ly, dx, dy):
        ...
        '''This function is used to create the coordinates for a 2-D grid. 
        '''
        lx = 2*lx1+lx2 + 2*dx #Length in x-dir (width)
...
class MainWindow(QMainWindow):
     def __init__(self):
        ...
        self.setWindowTitle("Stream Function Evaluator")
        self.form=Form()
...
class Form(QDialog):
    ...
    def updateUI(self):
        '''When this function is called the input parameters are considered and the grid points 
        as well as the PSI function are changed up. 
        '''
...
class MatplotlibCanvas(FigureCanvas):
    ...
    def redraw(self, x, y, PSI):
        '''This function "redraws" the contour plot 
        '''
        import matplotlib.pyplot as plt
        self.axes.clear()
